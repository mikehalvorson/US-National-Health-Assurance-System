...
R114 [S1]: this read the catalog from the retired tree and wrote the
scorecards back into it, while the live src/lib/data-phases.ts was a hand
port.  Regenerating updated the retired artifact and the methodology document
and silently left the live one stale, so the methodology would agree with a
file nothing serves and disagree with the deployed site.  It now reads
src/lib/quality-data.ts and writes src/lib/data-phases.ts.
"""
...
QUALITY_DATA = ROOT / "src" / "lib" / "quality-data.ts"
TS_OUTPUT = ROOT / "src" / "lib" / "data-phases.ts"
DOC_OUTPUT = ROOT / "research" / "data_phase_target_methodology.md"

# R298 [S12]: these track the wrapper extract_quality_catalog.py writes, which
# is now a type annotation rather than an `as unknown as` cast.  The slice is
# guarded - a wrapper that stops matching raises below rather than reading a
# short payload - so the coupling is loud.
QUALITY_PREFIX = "export const NHA_QUALITY_DATA: QualityData = "
QUALITY_SUFFIX = ";"

# The TypeScript around the payload.  R298 [S12] removed the
# `as unknown as { phases: DataPhase[] }` cast that used to sit under the
# literal.  It discarded four fields - both declared counts, the provenance
# stamp and the methodology path - and, because the assertion went through
# `unknown`, it also switched off type validation of every metric object in
# the payload.  The literal now carries a plain type ANNOTATION, so the
# compiler checks all 66 of them and the four fields are ordinary reads.
TS_HEADER = '''/* Data-tab rollout phase targets. GENERATED by
   tools/build_data_phase_targets.py from the controlled KPP/TPP catalog in
   src/lib/quality-data.ts and the explicit interim proposals maintained in
   that script.

   Regenerate with:  python tools/build_data_phase_targets.py
   Do not edit the payload by hand: the next regeneration discards the edit. */

import { PHASE_YEAR } from './rollout';

/* R298 [S12], review: `basis` was a bare `string`. The two values carry
   AK1/AN7's claim distinction - whether a target is a framework requirement or
   an analyst proposal - and the methodology says in terms that a derived
   target "must not be presented as" a framework requirement. A bare string
   cannot hold that; the union makes an invented third value a build error. */
export type DataBasis = 'framework' | 'derived';

export interface DataMetric { id: string; basis: DataBasis; name: string; phaseTarget: string; matureTarget: string; justification: string; }
...
/* Every top-level field the generator writes. The payload was previously cast
   to `{ phases: DataPhase[] }`, which named one of six and discarded the rest;
   naming all six is what lets them be read instead of re-cast one at a time,
   and what puts every metric object back under the type checker. */
export interface DataPhasePayload {
  source: string;
  methodologyPath: string;
  phases: DataPhase[];
  coverageGaps: DataCoverageGap[];
  rule3Metrics: Record<string, string>;
  metricCount: number;
  targetCount: number;
}

const NHA_DATA_PHASES: DataPhasePayload = '''
TS_MIDDLE = ''';

/* R251/R293 [S2]: the anchor years come from rollout.ts, the single phase
   map, rather than from this payload. The payload's own `year` fields are
   kept as the generator's record of what it wrote and are gated against
   PHASE_YEAR by a self-test, so a divergence fails the build instead of
   publishing two anchor years for one phase across two chapters. */
export const DATA_PHASES: DataPhase[] = NHA_DATA_PHASES.phases.map(function (p) {
  return PHASE_YEAR[p.id] === undefined ? p : { ...p, year: PHASE_YEAR[p.id] };
});
export const DATA_PHASE_YEARS_AS_GENERATED: Record<string, number> = {};
NHA_DATA_PHASES.phases.forEach(function (p) { DATA_PHASE_YEARS_AS_GENERATED[p.id] = p.year; });

/* R54 [§S0]: the file's own declared counts, reachable.
   They were unreachable because the literal was cast `as unknown as
   { phases: DataPhase[] }`, which discarded them. R298 [§S12] removed the
   cast, so these are now plain field reads rather than four more casts
   reaching back through `unknown` for what the type should have carried. */
export const DATA_PHASE_COUNTS: { metricCount: number; targetCount: number } = {
  metricCount: NHA_DATA_PHASES.metricCount,
  targetCount: NHA_DATA_PHASES.targetCount
};

/* R298 [§S12]: the provenance stamp and the methodology path, which the cast
   discarded along with the counts and which nothing could therefore check.
   The stamp matters because it is the claim about where these targets come
   from, and it is typed independently in two generators - here and in
   tools/extract_quality_catalog.py - so the two can disagree in the tree
   whenever one is regenerated and the other is not. */
export const DATA_PHASE_SOURCE: string = NHA_DATA_PHASES.source;
export const DATA_PHASE_METHODOLOGY_PATH: string = NHA_DATA_PHASES.methodologyPath;
...
export const DATA_PHASE_GAPS: DataCoverageGap[] = NHA_DATA_PHASES.coverageGaps;
...
export const DATA_PHASE_RULE3: Record<string, string> = NHA_DATA_PHASES.rule3Metrics;

/* R298 [S12], review: the row's acceptance test is "no consumer hardcodes the
   methodology path", and four did - data.astro twice and data-client.ts twice -
   while the path sat on this object as a field the cast had made unreachable.
   Built here rather than in methodology-check.ts because that module imports
   node:fs and data-client.ts is a client bundle.
   The repo base is still typed in ~20 other links across the site; that is a
   wider class than this row, and is not swept here. */
const REPO_BLOB = 'https://github.com/mikehalvorson/US-National-Health-Assurance-System/blob/main/';

export function methodologyUrl(anchor?: string): string {
  return REPO_BLOB + DATA_PHASE_METHODOLOGY_PATH + (anchor ? '#' + anchor : '');
}
'''
...
def load_quality_parameters() -> dict[str, dict]:
    ...
    try:
        start = raw.index(QUALITY_PREFIX) + len(QUALITY_PREFIX)
        end = raw.rindex(QUALITY_SUFFIX)
    except ValueError as exc:
        raise ValueError("Unrecognized quality-data.ts wrapper") from exc
    catalog = json.loads(raw[start:end])
...
def main() -> None:
    ...
    TS_OUTPUT.write_text(render(payload), encoding="utf-8")
    DOC_OUTPUT.write_text(markdown(payload), encoding="utf-8")
    print(
        f"Wrote {TS_OUTPUT.relative_to(ROOT)} and {DOC_OUTPUT.relative_to(ROOT)}: "
