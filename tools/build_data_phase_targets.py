...
QUALITY_PREFIX = "export const NHA_QUALITY_DATA = "
QUALITY_SUFFIX = " as unknown as QualityData;"

# The TypeScript around the payload.  DATA_PHASE_COUNTS exists because the
# payload is cast to { phases: DataPhase[] }, which discards the declared
# counts; exposing them is what lets a self-test compare the declared figure
# against the collection it describes (R54).
...
/* R54 [§S0]: the file's own declared counts, reachable.
   They were unreachable because the literal is cast `as unknown as
   { phases: DataPhase[] }`, which discards them (R110/R298, §S12 owns removing
   the cast). Exposing them here is what lets a self-test compare the declared
   figure against the collection it describes instead of trusting it. */
