...
- **⚠️ Neither cited URL carries the 7.0% / 7.5% figure. Measured 2026-08-27 (P16, §S10).** Both fetch 200. The NHE Fact Sheet page has no "net cost of health insurance" line at all, and `files/document/highlights.pdf` is *National Health Expenditures 2024 Highlights*, covering spending by type of service, by sponsor, and by major source of funds - not the administrative share. The number is real and lives in the NHE Tables archive (`https://www.cms.gov/files/zip/nhe-tables.zip`), which no pass has opened. **The HIGH grade above is on the evidence, not on the citation**, and this parameter is on `PRIORITY_EXEMPT` in `baseline-P16/migrate_seed.py` for exactly that reason: seeding it would mean inventing a citation. The dollar level of the same quantity *is* seeded, as `BL-0010`, $360.3B for 2023.
