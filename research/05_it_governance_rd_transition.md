...
### RB-05-IT-002: DOD MHS GENESIS EHR (military health system analogue)
...
### RB-05-IT-003: UK NHS National Programme for IT (NPfIT) - cautionary precedent
...
### RB-05-IT-004: National unique patient identifier (UPI) system - cost estimate
...
### RB-05-IT-005: Healthcare data breach cost (highest-cost sector, 14 consecutive years)
...
### RB-05-IT-006: Healthcare cybersecurity spending as % of IT budget
...
### RB-05-IT-007: Epic/Oracle Cerner large-system EHR licensing/implementation cost benchmarks
...
### RB-05-IT-008: ONC (Office of the National Coordinator for Health IT) federal budget
...
- **Confidence/caveat:** MEDIUM - ONC's own operating budget is a poor proxy for national interoperability infrastructure cost since it is a *policy coordination* office, not a system operator; it doesn't fund actual data infrastructure at scale. Useful mainly as a floor for "federal interoperability policy/standards-setting overhead," not for capital infrastructure costs (see RB-05-IT-004 for that).

### RB-05-IT-009: IBM Cost of Data Breach - general enterprise average (context/comparator)
...
- **Source:** Same IBM 2025 report as RB-05-IT-005.
...
### RB-05-GOV-002: CMS total "Program Management" discretionary budget
...
- **Confidence/caveat:** HIGH for the raw dollar figures; note this 0.3% ratio is *narrower* than the RB-05-GOV-001 figure because Program Management excludes claims-processing contractor payments (Medicare Administrative Contractors), which are booked elsewhere in Medicare Part A/B trust fund spending, not in the CMS discretionary line. Use RB-05-GOV-001 for "all-in Medicare administrative cost" and RB-05-GOV-002 for "core federal agency overhead" - they answer different questions and should not be summed naively.

### RB-05-GOV-003: SSA administrative cost ratio (federal benefit-administration analogue)
...
### RB-05-GOV-004: HHS Office of Inspector General (OIG) budget
...
### RB-05-GOV-006: National Health Expenditure - "Net cost of health insurance" (administrative overhead) as % of total U.S. health spending
...
- **Confidence/caveat:** HIGH - this is the authoritative top-line figure. **This is arguably the single most important calibration number for NHA governance costs**: it represents the entire current U.S. insurance-administration base (~$340B/year at 7% of $4.9T) that NHA's consolidated administrative apparatus would need to replace, ideally at a lower ratio. Combine with RB-05-GOV-007/008 below (single-payer administrative-cost projections) to model the plausible NHA administrative-cost range.
- **⚠️ Neither cited URL carries the 7.0% / 7.5% figure. Measured 2026-08-27 (P16, §S10).** Both fetch 200. The NHE Fact Sheet page has no "net cost of health insurance" line at all, and `files/document/highlights.pdf` is *National Health Expenditures 2024 Highlights*, covering spending by type of service, by sponsor, and by major source of funds - not the administrative share. The number is real and lives in the NHE Tables archive (`https://www.cms.gov/files/zip/nhe-tables.zip`), which no pass has opened. **The HIGH grade above is on the evidence, not on the citation**, and this parameter is on `PRIORITY_EXEMPT` in `baseline-P16/migrate_seed.py` (**in the audit working
set, not in this repo**) for exactly that reason: seeding it would mean inventing a citation. The dollar level of the same quantity *is* seeded, as `BL-0010`, $360.3B for 2023.

### RB-05-GOV-007: CBO single-payer ("Medicare for All") administrative cost projections
...
### RB-05-GOV-008: Urban Institute / RAND single-payer administrative-cost estimates (cross-check)
...
### RB-05-GOV-009: SSA disability appeals system - caseload/cost/timeline benchmarks (ombudsman/appeals analogue)
...
### RB-05-GOV-010: Taiwan National Health Insurance (NHI) administrative cost - foreign single-payer analogue
...
### RB-05-RD-003: BARDA (Biomedical Advanced Research and Development Authority) budget
...
### RB-05-RD-004: Total U.S. pharmaceutical industry R&D spending
...
### RB-05-RD-006: NIH Innovation Account / 21st Century Cures Act funding (precision medicine, BRAIN initiative)
...
- **Unit:** USD/year.
- **Source:** CRS Report R43341 (as cited in RB-05-RD-001 search results).
...
### RB-05-TRN-002: Medicare Part D implementation (2003-2006) - partial data
...
### RB-05-TRN-003: Trade Adjustment Assistance (TAA) - displaced-worker retraining cost-per-worker (best available DOL/GAO analogue)
...
### RB-05-TRN-004: Number of U.S. workers in health insurance administration/billing (transition-support population)
- **Value/range:** Broad U.S. insurance industry (all lines): **~2.98 million jobs (2023)**. Health & medical insurance carriers specifically (BLS NAICS 524114) employ a subset of this. Medical Records Specialists (BLS code 29-2072): **194,800 jobs (2024)**, median wage $50,250, from the BLS Occupational Outlook Handbook. Note that the OEWS May 2024 release gives **194,570** for the same code, and that is the figure the model's anchor table uses; the two are different BLS products, not a correction of one by the other. Insurance Claims and Policy Processing Clerks (BLS code 43-9041): **229,070 jobs (May 2024)**. Billing and Posting Clerks (BLS code 43-3021): **417,500 jobs (May 2024)**. Both are recorded in `research/workforce_transition_methodology.md` under "Official employment anchors", with the vintage on each row; the model applies a health-function exposure share to them rather than using them whole.
- **Unit:** Number of workers (headcount).
- **Source:** Statista aggregation of BLS data (https://www.statista.com/statistics/194233/aggregate-number-of-insurance-employees-in-the-us/); BLS OOH Medical Records Specialists (https://www.bls.gov/ooh/healthcare/medical-records-and-health-information-technicians.htm); BLS OEWS pages for codes 43-9041 and 43-3021 (https://www.bls.gov/oes/current/oes439041.htm, https://www.bls.gov/oes/2023/may/oes433021.htm). The May 2024 counts for both codes are recorded in `research/workforce_transition_methodology.md`.
- **RESOLVED (R29, §S9a).** The two counts this entry was missing are in the repository and have been for some time. `research/workforce_transition_methodology.md`, under "Official employment anchors", carries **43-9041 Insurance claims and policy-processing clerks: 229,070 (May 2024)** and **43-3021 Billing and posting clerks: 417,500 (May 2024)**, alongside **29-2072 Medical records specialists: 194,570 (May 2024)** and the two industry anchors the model actually uses. A later pass solved the blocker and this file was never updated, so the note above was still sending readers to re-chase it. **Do not re-run the retrieval.** The three occupation figures are stated on the Workforce chapter and checked against the anchor table by the build.
- **Confidence/caveat:** MEDIUM-LOW on the aggregate, and the reason is not retrieval. The occupation and industry counts overlap: a claims clerk may already sit inside direct-health-insurer industry employment, so summing adjacent codes double-counts. The **~500,000 to 1,000,000+** range quoted for total health-insurance-administration/billing employment remains an unverified secondary-source band and is not a cited number. The model does not use it. It applies a functional exposure share to a single named anchor per category and counts each function once, which is the point of the `boundary` field on every legacy category.

### RB-05-TRN-005: Taiwan NHI transition timeline (foreign analogue, cross-reference to RB-05-GOV-010)
- **Value/range:** ~7-9 years from policy decision (1986) to launch (1995) for a much smaller, more centralized country than the U.S.
- **Source:** Same as RB-05-GOV-010.
...
### RB-05-TRN-006: NPfIT and VA EHRM as transition-risk cautionary precedents (cross-reference)
- **Value/range:** See RB-05-IT-001 and RB-05-IT-003. Both are examples of large-scale, centrally-mandated health IT/system transitions running 1.5-2x over initial budget and, in NHS's case, being abandoned outright after ~9 years and £10B+ spent.
...
### RB-05-IT-NEW-001: Healthcare cybersecurity workforce shortage (staffing cost driver)
- **Finding:** HIMSS 2024/2025 survey reporting (referenced in RB-05-IT-006 search) indicates healthcare cybersecurity budgets are rising specifically because qualified cybersecurity staff are hard to find/retain in the sector ("Healthcare cybersecurity budgets are rising, but workers are hard to find," Chief Healthcare Executive, https://www.chiefhealthcareexecutive.com/view/healthcare-cybersecurity-budgets-are-rising-but-workers-are-hard-to-find).
...
### RB-05-GOV-NEW-001: CMS Program Management vs. "fully loaded" administrative cost - reconciliation note
- **Finding:** As detailed under RB-05-GOV-002, CMS's own discretionary Program Management budget ($4.3B) is a much narrower slice than the commonly-cited "1.3% of Medicare spending" administrative cost figure (RB-05-GOV-001, ~$10.8B), because the latter includes Medicare Administrative Contractor (MAC) claims-processing payments that are booked against the trust funds rather than CMS's discretionary appropriation.
...
- **Not adopted, and why (R30, §S11a, recorded 2026-08-26).** The model does
  ...
  **What this note still gets right and the model does not do:** it is correct
  ...
  which of the two they are holding. That reconciliation lives in `RB-05-GOV-001`
  and `RB-05-GOV-002` and is not restated in the parameter.

### RB-05-RD-NEW-001: NIH budget political volatility as a modeling input
- **Finding:** As shown in RB-05-RD-001, the NIH budget swung from a proposed 40% cut (FY2026 President's Budget) to a Congressional increase within the same fiscal cycle (Jan 2026 appropriations deal). This is a documented, current (2025-2026) example of extreme year-to-year federal biomedical R&D funding volatility tied to administration changes.
...
### RB-05-TRN-NEW-001: BLS data access constraint - CLOSED (R29, §S9a)
- **Status:** Closed. The retrieval this note asked for was done by a later pass and the counts live in `research/workforce_transition_methodology.md` under "Official employment anchors". Leaving the note open cost the next reader a re-chase of a solved problem, which is the whole reason it is closed here rather than deleted: a closed note says the answer exists, a deleted one says nothing.
- **What remains true:** BLS.gov's OEWS occupation pages return HTTP 403 to generic automated fetchers, so a future pull needs the BLS public API or a real browser session rather than a plain fetch. That is a tooling constraint, not an open data gap.
- **What is no longer true:** that the counts are missing. They are not, and no work item follows from this entry.
...
|---|---|---|---|
| RB-05-GOV-006 | Total U.S. health-insurance administrative overhead (current system) | 7.0% of $4.9T NHE (2024) ≈ $340B/yr | HIGH |
| RB-05-GOV-007 | CBO single-payer administrative cost target | 1.5-2.0% of total spending | HIGH |
| RB-05-GOV-008 | Urban Institute/RAND single-payer administrative cost (more conservative) | 5-6% of total spending | MEDIUM-HIGH |
| RB-05-GOV-003 | SSA administrative cost ratio (universal benefit analogue) | 0.3-0.5% (OASI/OASDI) | HIGH |
| RB-05-IT-003 | NHS NPfIT cost-overrun cautionary precedent | £10-12.7B vs £6.2B budget (~2x overrun) | HIGH |
| RB-05-IT-001 | VA EHRM total lifecycle cost | $16B-$50B (wide, unresolved) | LOW-MEDIUM |
| RB-05-IT-005 | Healthcare data breach average cost | $7.42M (2025) | HIGH |
| RB-05-RD-001 | NIH annual budget | $47.0B (FY2025 enacted) | HIGH |
| RB-05-RD-004 | US pharmaceutical industry R&D spend | $83-105B/yr (methodology-dependent) | MEDIUM |
| RB-05-TRN-003 | TAA retraining cost per displaced worker | ~$10,000/worker | MEDIUM |
| RB-05-IT-004 | National patient identifier system cost | $1.5-11.1B one-time (2008 dollars) | MEDIUM |
